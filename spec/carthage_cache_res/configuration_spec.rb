...
describe CarthageCacheRes::Configuration do

  describe 'defaults' do
    it 'has ~/Library/Caches/carthage_cache_res as the default temp directory' do
      expect(CarthageCacheRes::Configuration.default.tmpdir).to eq File.join(Dir.home, 'Library', 'Caches')
    end
  end
  ...
  describe '#merge' do
    ...
    it 'merges keys from another configuration object into this one correctly' do
      ...
      other_config = CarthageCacheRes::Configuration.new
      other_config.hash_object[:foo]="bar"
      main_config = CarthageCacheRes::Configuration.new
