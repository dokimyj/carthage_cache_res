...
describe CarthageCacheRes::Archiver do
  ...
  describe "#archive" do
    ...
    it "creates a zip file with the content of the project's 'Carthage/Build' directory" do
      expected_command = "cd #{build_directory} && zip -r -X #{archive_path} CarthageCacheRes.lock iOS Mac tvOS watchOS > /dev/null"
    ...
    context "when a filter block is passed" do
      ...
      it "filters platforms that don't match the filter" do
        expected_command = "cd #{build_directory} && zip -r -X #{archive_path} CarthageCacheRes.lock iOS > /dev/null"
        expect(executor).to receive(:execute).with(expected_command)
        archiver.archive(build_directory, archive_path) do |x|
          x == CarthageCacheRes::CarthageCacheResLock::LOCK_FILE_NAME || x == "iOS"
        end
      end
