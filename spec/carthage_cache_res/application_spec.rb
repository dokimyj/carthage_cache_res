...
describe CarthageCacheRes::Application do
  ...
  let(:tmpdir) { File.join(TMP_PATH, "carthage_cache_res") }
  let(:archive_path) { File.join(tmpdir, archive_filename) }
  subject(:application) { CarthageCacheRes::Application.new(FIXTURE_PATH, false, { tmpdir: TMP_PATH }, options) }
